...
- **Breaking.** `PppCorrectionLookup::with_ssr_biases` takes the `SsrCorrectedEphemeris` the solve uses instead of an `SsrCorrectionStore`, and an approximate receiver position; it returns `(Self, SsrPppBiasApplicationReport)`. It predicts each observation's transmission time from the reception time and that position, as the solve does, and resolves biases there with `query_code_bias` and `query_phase_bias` instead of the untimed `code_bias` and `phase_bias`, so validity, staleness, HAS do-not-use exclusion and phase continuity reach the PPP rows. HAS validity runs forward from the reference epoch (HAS SIS ICD 5.2.2.1, 7.7), so a signal that left before a TOH does not use that TOH's corrections even when it arrives after it. The report holds an `SsrPppAggregateStatus`, applied and failed counts for code and phase, the number of phase observations to reset, and one `SsrObsApplicationReport` per requested observation, applied or not, with its transmission time, the orbit and clock solution the source applies there, each signal's query result in an `SsrObsSignalReport`, and an `SsrIfCombinationStatus` for code and for phase; `PppCorrectionLookup` gains `ssr_bias_report`, a copy of it. A bias is applied only when the source applies SSR orbit and clock corrections to the satellite at the transmission time and both signals' biases come from the same source, provider and solution as those corrections. A bias from another solution is reported as `OrbitClockSolutionMismatch`; where the source declines the satellite or falls back to the broadcast state it is reported as `OrbitClockSolutionUnavailable`; a satellite a HAS do-not-use indication excludes is reported as `SatelliteExcluded`, with the indication in each signal's query details; one the source cannot place for another reason as `TransmitTimeUnavailable`. None of these is applied. Biases were previously applied with no check against the orbit and clock, so one SSR solution's biases could be combined with another solution's clocks or with broadcast clocks.
- **Breaking.** The static float solve iterates the SSR/HAS bias exclusion to a fixed point. When the recorded biases of an observation stop holding at the transmission time of an iteration, including one of the residual screen's re-solves, or do not hold at the position a pass converged to for an observation the solution kept, the observation is excluded and the solve starts again from the state it reached. `SsrBiasExclusion::pass` records the pass that found each exclusion, 0 for the check from the starting position, and the new `SsrBiasExclusion::stage` (`SsrBiasExclusionStage::{BeforeSolve, DuringIteration, AtConvergence}`) where. Once a pass converges with nothing new to exclude, every exclusion made for a transmit-time reason is checked again at the converged position and admitted again, once each, when it holds; an observation that fails again stays excluded. The biases `with_ssr_biases` resolved at its `receiver_position_m` are re-checked from the solve's own starting position. An observation admitted again is not checked at intermediate states, including the check from the starting state of each later pass; the check at convergence decides. The fixed solve starts from the float solution's exclusions and leaves out the observations its residual screen removed; `FloatSolution` gains `ssr_bias_readmissions`, `ssr_bias_last_pass` (the pass the float solve converged on: 1 when its first pass converged with nothing to exclude, 0 for a float solution the SSR/HAS fixed point did not produce), `residual_screen`, `solve_options` and `residual_screen_removals` to carry them. The fixed solve refuses a float solution whose `solve_options` the float solve would refuse, whose `residual_screen_removals` or `ssr_bias_readmissions` name an input epoch past the arc or an ambiguity that epoch does not observe, or whose `ssr_bias_last_pass` or any exclusion's `pass` exceeds `u32::MAX`; a pass counter that would overflow is refused as `InvalidInput`. When the float solution left observations out, including residual screen removals, the integer search orders the ambiguities the fixed arc still observes. When the fixed arc observes an ambiguity the float solution did not solve, such as a satellite the float solve's elevation cutoff removed and a lower fixed cutoff, or none, keeps, the fixed solve solves the float arc again over the fixed arc, once for each such ambiguity, and fixes again. As in every float re-solve the fixed solve makes, the float arc is solved with the fixed configuration's weights, troposphere, corrections, residual ionosphere setting and elevation cutoff, with the caller's float solve options and residual screen setting, and the residual screen runs again over the whole arc and replaces the caller's removals. A satellite within numerical noise of the cutoff can come back unsolved after the re-solve, and the fixed solve then fails with `MissingAmbiguity`. The fixed solve previously failed with `MissingAmbiguity` in this case, or panicked for a float solution holding the ambiguity's value without listing it in `used_sats`. In that re-solve an ambiguity the float solution carries no value for starts from phase minus code of its first observation. When a bias stops holding during the fixed re-solve (`SsrBiasExclusionStage::FixedResolve`), or an observation admitted again fails at the fixed position, it excludes that observation, solves the float arc again to its fixed point from the float solution's state with the float solve's residual screen setting and options, and fixes again; `FixedSolution::float_solution` is then that re-solved float. After a fix succeeds, every exclusion made for a transmit-time reason, by the float solve or by the fixed re-solve, is checked at the fixed position; those that hold are admitted again, once each, the float arc is solved again with them and fixed again. An observation admitted again after a fix and excluded at a later fixed position is not admitted again. Pass numbers continue past the float solution's last pass and every exclusion found so far; an exclusion found while the ambiguity covariance for the integer search is formed is labelled `FixedResolve`, and an exclusion found while the rows of a converged state are assembled is labelled `AtConvergence`. SSR exclusion runs before the elevation cutoff, so a satellite the source cannot place is left out instead of failing the cutoff's geometry, and each step checks the observations left for the solve and names itself when they do not suffice. The sufficiency check requires four distinct satellites, where it counted ambiguity arcs.
...
- **Breaking.** The static float, fixed and kinematic PPP rows, the elevation cutoff and the RAIM geometry take the transmit-time geometry from the new `observables::predict_transmit_geometry` instead of `predict`. It returns a `TransmitGeometry` (flight time, transmit offset and epoch, satellite clock, rotated satellite position, range, line of sight, elevation, azimuth) without evaluating the satellite velocity, and its light-time iteration starts from the observation's pseudorange over the speed of light (`observables::flight_time_seed_s`, with `NOMINAL_SIGNAL_FLIGHT_TIME_S`, 0.075 s, where the pseudorange is not a positive finite distance). The seed only picks where the source is first queried, near the transmission time rather than at the reception time; the iteration converges to the same transmission time. The model forms the satellite velocity, with `observables::transmit_velocity_m_s`, only for the satellite clock relativity term when that is enabled. `predict` formed the velocity by differencing the source half a second either side of the transmission epoch, so a solve failed for any observation transmitted within half a second after an SSR correction's reference epoch on a source that declines the satellite before it, and blended a broadcast and an SSR position on one that falls back. `predict` itself still starts the iteration at the reception time. `observables::transmit_epoch_j2000_s` returns the transmission epoch from the same iteration alone.
