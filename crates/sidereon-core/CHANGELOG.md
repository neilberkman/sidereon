...
- **Breaking.** SPP, the static solve, DGNSS and every solve seeded from SPP weight each pseudorange by the inverse of the variance RTKLIB `rescode` forms with the `prcopt_default` options, in place of `sin^2(el) / sigma0^2`: the ephemeris variance of the satellite's state, the code bias variance `0.3^2` m² of a single-frequency code (none for the ionosphere-free code), the ionosphere variance, the troposphere variance and the `varerr` code error `EFACT^2 * 300^2 * (0.003^2 + 0.003^2 / sin(el))` with `el` no lower than 5 degrees (`EFACT` 1.5 for GLONASS and NavIC, 3 for SBAS, 1 otherwise; nine times that for the ionosphere-free code). The ionosphere variance is `(0.5 * delay)^2` for a broadcast model, the augmentation grid's DO-229 slant variance for a grid, `5^2 * (f_L1 / f)^4` m² with the ionosphere uncorrected on a single-frequency code and none on the ionosphere-free code; the troposphere variance is `(0.3 / (sin(el) + 0.1))^2` with the troposphere corrected and `3^2` m² without. `SIGMA0_M` is removed; the new constants `CODE_PHASE_ERROR_RATIO`, `PHASE_ERROR_BASE_M`, `PHASE_ERROR_ELEVATION_M`, `ERROR_MODEL_MIN_ELEVATION_RAD`, `CODE_BIAS_ERROR_M`, `UNCORRECTED_IONOSPHERE_ERROR_M`, `BROADCAST_IONOSPHERE_ERROR_FACTOR`, `UNCORRECTED_TROPOSPHERE_ERROR_M` and `TROPOSPHERE_MODEL_ERROR_M` state the model. Every SPP, static, DGNSS and SPP-seeded position moves; against RTKLIB demo5 `pntpos` on the ESBC and WTZR fixtures, 2400 solves from four initial positions, the largest distance falls from 1.39 m to 0.474 m with the troposphere corrected and from 4.14 m to 8.1e-5 m without it. The 0.474 m is the troposphere model (Saastamoinen with the solve's surface meteorology and Niell mapping, where RTKLIB takes `tropmodel`), and the 8.1e-5 m the `f64` transmission epochs, rounded to 1.2e-7 s where RTKLIB `gtime_t` keeps the fraction of the second; with the position shifts both predict taken out, the solutions agree to 7.2e-4 m and 3.4e-8 m. At RTKLIB's position the covariance restates RTKLIB's `Q` to its single precision. The oracle (`fixtures-generators/rtklib_spp_oracle`) gains two runs with the troposphere uncorrected and records RTKLIB's covariance.
- **Breaking.** SPP, the static solve, DGNSS and every solve seeded from SPP weight each pseudorange by the inverse of the variance RTKLIB `rescode` forms with the `prcopt_default` options, in place of `sin^2(el) / sigma0^2`: the ephemeris variance of the satellite's state, the code bias variance `0.3^2` m² of a single-frequency code (none for the ionosphere-free code), the ionosphere variance, the troposphere variance and the `varerr` code error `EFACT^2 * 300^2 * (0.003^2 + 0.003^2 / sin(el))` with `el` no lower than 5 degrees (`EFACT` 1.5 for GLONASS and NavIC, 3 for SBAS, 1 otherwise; nine times that for the ionosphere-free code). The ionosphere variance is `(0.5 * delay)^2` for a broadcast model, the augmentation grid's DO-229 slant variance for a grid, `5^2 * (f_L1 / f)^4` m² with the ionosphere uncorrected on a single-frequency code and none on the ionosphere-free code; the troposphere variance is `(0.3 / (sin(el) + 0.1))^2` with the troposphere corrected and `3^2` m² without. `SIGMA0_M` is removed; the new constants `CODE_PHASE_ERROR_RATIO`, `PHASE_ERROR_BASE_M`, `PHASE_ERROR_ELEVATION_M`, `ERROR_MODEL_MIN_ELEVATION_RAD`, `CODE_BIAS_ERROR_M`, `UNCORRECTED_IONOSPHERE_ERROR_M`, `BROADCAST_IONOSPHERE_ERROR_FACTOR`, `UNCORRECTED_TROPOSPHERE_ERROR_M` and `TROPOSPHERE_MODEL_ERROR_M` state the model. Every SPP, static, DGNSS and SPP-seeded position moves; against RTKLIB demo5 `pntpos` on the ESBC and WTZR fixtures, 2400 solves from four initial positions, the largest distance falls from 1.39 m to 8.2e-5 m with the troposphere corrected (with the RTKLIB troposphere below) and from 4.14 m to 8.1e-5 m without it. What remains is the `f64` transmission epochs, rounded to 1.2e-7 s where RTKLIB `gtime_t` keeps the fraction of the second; with the position shift that rounding predicts taken out, the solutions agree to 2.2e-7 m, where each solve stops short of its fixed point. At RTKLIB's position the covariance restates RTKLIB's `Q` to its single precision. The oracle (`fixtures-generators/rtklib_spp_oracle`) gains two runs with the troposphere uncorrected and records RTKLIB's covariance.
- **Breaking.** `EphemerisSource` gains `ephemeris_variance_m2(sat, t_j2000_s, selection_j2000_s)`, the satellite position and clock variance RTKLIB `satposs` returns, `0.0` by default. A `BroadcastEphemeris` returns `var_uraeph` of the selected record's accuracy (the Galileo SISA through `sisa_index`, any other system's URA through `uraindex`, a blank accuracy read as 0), 25 m² for GLONASS and the URA variance of an SBAS record, and `BroadcastStore::ephemeris_variance_m2` gives it directly. An SSR-corrected source returns `var_urassr` of the satellite's SSR URA (`0.15^2` m² without one) and the broadcast variance for a broadcast fallback state. An SBAS-corrected source returns the variance of the fast correction it applies as `sbsfastcorr` states it, the UDRE variance plus `degfcorr(ai) t^2 / 2` with the message type 7 degradation indicator, which the correction store now keeps, and the broadcast variance where it applies no fast correction. The SP3 and precise-interpolant sources keep `0.0`: RTKLIB `peph2pos` forms its variance from the SP3 records' standard deviations, which `Sp3` does not keep, and from its linear clock interpolation, where these sources interpolate the clock by spline.
- `SbasIonoGrid::slant_variance_m2` gives the variance of `slant_delay_m`: the interpolated GIVE variance times the square of the obliquity factor, scaled to the carrier.
