...
- SSR ingestion of HAS messages no longer leaves an earlier correction in place when a later message withdraws it. An orbit record with any component transmitted as unavailable removes the satellite's stored orbit correction, which may belong to another IOD; a clock record transmitted as unavailable or marked do-not-use removes its stored clock correction, since the service has excluded the satellite or has no correction for it; and a code or phase bias transmitted as unavailable removes that signal's stored bias. No zero is stored in place of a withdrawn correction, and other satellites and signals keep theirs. A phase bias on a signal with no assigned carrier is skipped, with no wavelength assumed for it; such a message previously failed to decode and now applies everything else. Every validity interval in the message is read before any record is applied, so a reserved one leaves the store as it was.
