...
//! values in the committed golden fixture `tests/fixtures/troposphere_golden.json`.
...
#[test]
fn zwd_xyz_variant_matches_proj_reference() {
    ...
    assert_eq!(
        doc["elevation_reference"],
        "PROJ topocentric transform from WGS84 ECEF to receiver ENU"
    );
    assert!(doc["pyproj_version"].as_str().is_some());
    assert!(doc["proj_version"].as_str().is_some());

    let tolerance_m = 1.5e-8;
    ...
    for case in doc["cases"].as_array().expect("cases") {
        ...
        let longitude = receiver_lonlatalt[0].to_radians();
        let latitude = receiver_lonlatalt[1].to_radians();
        let east = [-libm::sin(longitude), libm::cos(longitude), 0.0];
        let north = [
            -libm::sin(latitude) * libm::cos(longitude),
            -libm::sin(latitude) * libm::sin(longitude),
            libm::cos(latitude),
        ];
        let up = [
            libm::cos(latitude) * libm::cos(longitude),
            libm::cos(latitude) * libm::sin(longitude),
            libm::sin(latitude),
        ];
        let line_of_sight = [sat[0] - rx[0], sat[1] - rx[1], sat[2] - rx[2]];
        let dot = |first: [f64; 3], second: [f64; 3]| {
            first[0] * second[0] + first[1] * second[1] + first[2] * second[2]
        };
        let horizontal_east = dot(line_of_sight, east);
        let horizontal_north = dot(line_of_sight, north);
        let vertical = dot(line_of_sight, up);
        let range = libm::sqrt(dot(line_of_sight, line_of_sight));
        let elevation = libm::atan2(vertical, libm::hypot(horizontal_east, horizontal_north));
        assert!(range >= 20.0e6, "{name} range is within the error bound");
        assert!(
            elevation.to_degrees() > 4.8 && elevation.to_degrees() < 84.2,
            "{name} elevation is within the error bound: {} degrees",
            elevation.to_degrees()
        );
        let geocentric_up = crate::astro::math::vec3::unit3_ref_unchecked(&rx);
        let geodetic_up = up;
        let geocentric_elevation = libm::asin(
            dot(
                geocentric_up,
                [
        ...
        let up_separation = libm::atan2(
            libm::sqrt(
                (geocentric_up[1] * geodetic_up[2] - geocentric_up[2] * geodetic_up[1]).powi(2)
                    + (geocentric_up[2] * geodetic_up[0] - geocentric_up[0] * geodetic_up[2])
                        .powi(2)
                    + (geocentric_up[0] * geodetic_up[1] - geocentric_up[1] * geodetic_up[0])
                        .powi(2),
            ),
            dot(geocentric_up, geodetic_up),
        );
        assert!(
            (elevation - geocentric_elevation).abs() <= up_separation + 1.0e-12,
            "{name} mixed geocentric/geodetic generator direction is accounted for"
        );
        ...
        assert!(
            (got - want).abs() <= tolerance_m,
            "{name} ZWD delay: got={got:.15e} want={want:.15e}"
        );
        checked += 1;
    }
