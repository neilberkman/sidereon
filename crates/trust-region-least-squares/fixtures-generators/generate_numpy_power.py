...
* ``scalar_bits`` -- ``np.float64(base) ** exponent`` element by element. NumPy
  ...
  ``ThinSvd::power_scalar`` must reproduce.
...
(``-0.5``, ``-1.5``), the ``denom ** 3`` hook, and neighbours that exercise the
odd-integer / non-integer branches of the C ``pow`` special cases.
