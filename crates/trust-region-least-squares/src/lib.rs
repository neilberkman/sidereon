...
//! operations that determine the last bits of the trajectory -- the thin SVD
...
//!
//! Every hook on [`trf::HostNumerics`] except the SVD itself is a defaulted
//! `Ok(None)`, so an implementation only overrides what it actually needs to
//! pin and otherwise keeps the pure-Rust computation.
//!
...
//! - [`hostlapack`]: a [`trf::HostNumerics`] implementation backed by a dynamically
...
//! [`trf::HostNumerics`]). The iteration is identical, only the host-numerics seam
//! changes.
//!
