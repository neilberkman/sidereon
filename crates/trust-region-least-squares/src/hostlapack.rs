//! Host LAPACK, BLAS, and NumPy power bridge for pinned SciPy parity runs.
//!
//! This module resolves symbols from a configured dynamic library and exposes
//! the thin SVD, small BLAS operations, and power dispatch used by the
//! trust-region solver.
...
/// A [`trf::HostNumerics`] backed by a host LAPACK/BLAS/NumPy runtime.
///
...
impl trf::HostNumerics for LapackSvd {
    fn svd(
        &self,
        a: &[f64],
        m: usize,
        n: usize,
    ) -> Result<(Vec<f64>, Vec<f64>, Vec<f64>), trf::BackendError> {
        let result = match &self.path {
            Some(path) => thin_svd_with_lapack_path(path, m, n, a),
            None => thin_svd_from_env(m, n, a),
        }
        .map_err(trf::BackendError::from)?;
        Ok((result.u, result.s, result.vt))
    }

    fn dot(&self, a: &[f64], b: &[f64]) -> Result<Option<f64>, trf::BackendError> {
        let path = self.resolve_blas_path().map_err(trf::BackendError::from)?;
    ...
    fn fortran_matvec(
        ...
        x: &[f64],
        transpose: bool,
    ) -> Result<Option<Vec<f64>>, trf::BackendError> {
        let path = self.resolve_blas_path().map_err(trf::BackendError::from)?;
    ...
    fn power(&self, values: &[f64], exponent: f64) -> Result<Option<Vec<f64>>, trf::BackendError> {
        let path = self.resolve_blas_path().map_err(trf::BackendError::from)?;
    ...
    fn power_scalar(&self, base: f64, exponent: f64) -> Result<Option<f64>, trf::BackendError> {
        let path = self.resolve_blas_path().map_err(trf::BackendError::from)?;
...
impl From<LapackError> for trf::BackendError {
    fn from(value: LapackError) -> Self {
        trf::BackendError::Failed(value.to_string())
    }
}
