...
//!
//! `HostNumerics::power` (elementwise `values ** exponent`) and
//! `HostNumerics::power_scalar` (scalar `base ** exponent`) let a host backend supply
//! the exact results an external numerical runtime produces for the routed
//! expressions: the robust-loss derivative powers (`z ** -0.5`, `z ** -1.5`),
//! the trust-region derivative's `denom ** 3`, and the trust-region alpha seed
...
use trust_region_least_squares::trf::{
    BackendError, HostNumerics, NalgebraThinSvd, TrfError, TrfOptions, TrfResult,
};

/// A backend written before the power hooks existed: it implements only the one
/// required `HostNumerics` method. It must keep compiling and keep producing the
/// crate's existing results through the trait defaults.
struct SvdOnlyBackend;

impl HostNumerics for SvdOnlyBackend {
    fn svd(
        &self,
        a: &[f64],
        m: usize,
...
impl HostNumerics for RecordingBackend {
    fn svd(
        &self,
        a: &[f64],
        m: usize,
...
impl HostNumerics for TooLongPowerBackend {
    fn svd(
        &self,
        a: &[f64],
        m: usize,
    ...
    fn power(&self, values: &[f64], _exponent: f64) -> Result<Option<Vec<f64>>, BackendError> {
...
fn solve<M: ResidualModel>(
    ...
    solve_model_with(model, x0, host, options)
}
...
fn robust_solve_dispatches_the_vector_power_hook() {
    for (loss, expected_exponents) in [
        ...
        let mut exponents: Vec<f64> = calls
            .iter()
...
#[test]
fn trust_region_derivative_routes_exponent_three_through_the_unified_hook() {
    let recorder = RecordingBackend::default();
    solve(
        &RankDeficientModel,
        &[0.0, 0.0],
    ...
    assert!(
        calls
            .iter()
            .any(|(values, exponent)| { *exponent == 3.0 && !values.is_empty() }),
        "the trust-region derivative never dispatched exponent 3.0: {calls:?}"
    );
}
...
fn host_power_failure_propagates_as_a_typed_error() {
    ...
    impl HostNumerics for FailingBackend {
        fn svd(
            &self,
            a: &[f64],
            m: usize,
        ...
        fn power(&self, _values: &[f64], _exponent: f64) -> Result<Option<Vec<f64>>, BackendError> {
            Err(BackendError::Failed("no host power".to_string()))
        }
    }
...
fn supplied_host_power_results_reach_the_rho_output() {
    ...
    impl HostNumerics for ConstantPowerBackend {
        fn svd(
            &self,
            a: &[f64],
            m: usize,
...
fn scalar_power_failure_propagates_out_of_the_solve() {
    ...
    impl HostNumerics for FailingScalarBackend {
        fn svd(
            &self,
            a: &[f64],
            m: usize,
        ...
        fn power_scalar(&self, _base: f64, _exponent: f64) -> Result<Option<f64>, BackendError> {
            Err(BackendError::Failed("no host scalar power".to_string()))
        }
    }
